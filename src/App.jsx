...
import { 
  ...
  signInWithRedirect, getRedirectResult, sendSignInLinkToEmail, isSignInWithEmailLink, signInWithEmailLink 
} from 'firebase/auth'; 
...
export default function App() {
  ...
  useEffect(() => {
    const initAuth = async () => {
      // 1. Check for Redirect Login Result
      try {
        ...
        setShowAccountModal(true);
        setAuthMessage({ type: 'error', text: "Login failed: " + err.message });
      }

      // 2. Check for Magic Link
  ...
  const handleGoogleLogin = async () => {
    try {
      setAuthMessage({ type: 'info', text: 'Redirecting to Google...' });
      const provider = new GoogleAuthProvider();
      // Using redirect instead of popup to avoid Vercel COOP errors
      ...
      {showAccountModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl p-6 sm:p-8 w-full max-w-md border-4 border-slate-200 relative">
            ...
            {/* CONDITIONAL RENDER: Logged In vs Not Logged In */}
            {isAccountLinked ? (
