// PASTE YOUR ENTIRE 495-ROW CSV HERE!
// Make sure it starts with the "Number,Third-placed..." header and ends with row 495.
const rawCsvData = `Number,Third-placed teams advance from groups,,,,,,,,,,,,1A vs,1B vs,1D vs,1E vs,1G vs,1I vs,1K vs,1L vs
...
// --- THE PARSER ENGINE ---
// This safely reads your CSV string and turns it into a high-speed dictionary!
const parseThirdPlaceMap = (csvString) => {
    const lines = csvString.trim().split('\n');
    const map = {};
    
    // Start at index 1 to skip the header row
    for (let i = 1; i < lines.length; i++) {
        // Strip out any weird carriage returns from Excel
        const line = lines[i].replace(/\r/g, '').trim();
        if (!line) continue;
        
        const cols = line.split(',');
        
        // Grab Columns 1-12 (The advancing groups) and combine them into a key like "EFGHIJKL"
        const advanced = cols.slice(1, 13).map(c => c.trim()).filter(c => c !== '').join('').toUpperCase();
        
        if (advanced.length === 8) {
            // Grab Columns 13-20 and strip the "3" out so we just have the letter "E", "J", etc.
            map[advanced] = {
                '1stA': cols[13]?.replace('3', '').trim().toUpperCase(),
                '1stB': cols[14]?.replace('3', '').trim().toUpperCase(),
                '1stD': cols[15]?.replace('3', '').trim().toUpperCase(),
                '1stE': cols[16]?.replace('3', '').trim().toUpperCase(),
                '1stG': cols[17]?.replace('3', '').trim().toUpperCase(),
                '1stI': cols[18]?.replace('3', '').trim().toUpperCase(),
                '1stK': cols[19]?.replace('3', '').trim().toUpperCase(),
                '1stL': cols[20]?.replace('3', '').trim().toUpperCase(),
            };
        }
    }
    return map;
...
export const THIRD_PLACE_MAP = parseThirdPlaceMap(rawCsvData);