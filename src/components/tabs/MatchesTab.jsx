      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4 md:gap-8">
        ...
        <div className="w-full md:w-1/3 flex flex-col gap-2">
          <div className="bg-white/95 backdrop-blur-sm p-2 md:p-3 rounded-xl shadow-lg border border-emerald-100 flex items-center justify-between w-full relative overflow-hidden">
            ...
            <div className="flex flex-col w-full relative z-10">
              {isKnockout && !isViewer ? (
                <select value={match.teamA} onChange={(e) => handleMatchUpdate(match.id, 'teamA', e.target.value)}
        ...
        <div className="flex flex-col items-center gap-2 bg-white/95 backdrop-blur-sm px-5 py-3 rounded-2xl border-2 border-emerald-100 shadow-xl min-w-[180px]">
            ...
            <div className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex flex-col items-center gap-0.5 text-center">
              ...
              {match.location && (
                <span className="flex items-center justify-center gap-1 mt-0.5 text-slate-400/80 leading-tight">
                  <MapPin className="w-3 h-3 shrink-0 self-start mt-0.5" /> 
                  <span className="max-w-[160px] text-wrap text-center">{match.location}</span>
                </span>
              )}
            </div>
          )}
          ...
          <div className="flex items-center gap-3 mt-1">
            {isViewer ? (
              ...
              <>
                <input type="number" min="0" value={match.scoreA} onChange={(e) => handleMatchUpdate(match.id, 'scoreA', e.target.value)}
                ...
                <span className="text-slate-300 font-black text-xl">-</span>
                <input type="number" min="0" value={match.scoreB} onChange={(e) => handleMatchUpdate(match.id, 'scoreB', e.target.value)}
                       className="w-12 h-12 text-center bg-slate-100 border-2 border-slate-200 rounded-lg font-black text-2xl text-slate-800 focus:border-emerald-500 focus:bg-white focus:outline-none transition-all shadow-inner" />
              </>
            )}
          </div>
        ...
        <div className="w-full md:w-1/3 flex flex-col gap-2">
          <div className="bg-white/95 backdrop-blur-sm p-2 md:p-3 rounded-xl shadow-lg border border-emerald-100 flex items-center justify-between w-full relative overflow-hidden">
            ...
            <div className="flex flex-col w-full relative z-10">
              {isKnockout && !isViewer ? (
                <select value={match.teamB} onChange={(e) => handleMatchUpdate(match.id, 'teamB', e.target.value)}
                        className="p-1 border border-slate-200 rounded text-sm w-full font-bold text-slate-800 focus:border-emerald-500 focus:outline-none mb-1 shadow-sm bg-white cursor-pointer">
                  <option value="">-- {match.labelB || 'TBD'} --</option>
                  {eligibleTeamsB.map(t => (
                     <option key={t.id} value={t.id}>{t.id} - {t.name}</option>
                  ))}
                </select>
              ) : (
...
export const MatchesTab = ({ 
  ...
  const [matchFilter, setMatchFilter] = useState(() => {
    return localStorage.getItem('worldCupGroupFilter') || 'All';
  });
  ...
  const toggleMain = () => {
    const next = { ...uiState, main: !uiState.main };
    setUiState(next);
    localStorage.setItem(storageKey, JSON.stringify(next));
  };

  const toggleGroup = () => {
    const next = { ...uiState, group: !uiState.group };
    setUiState(next);
    localStorage.setItem(storageKey, JSON.stringify(next));
  };

  const toggleStage = (stageId) => {
    const next = { ...uiState, [stageId]: !uiState[stageId] };
    setUiState(next);
    localStorage.setItem(storageKey, JSON.stringify(next));
  };

  const toggleSortFinished = () => {
    const next = { ...uiState, sortFinishedBottom: !uiState.sortFinishedBottom };
    setUiState(next);
    localStorage.setItem(storageKey, JSON.stringify(next));
  };
  ...
  return (
    <div className="space-y-8 animate-fade-in relative pb-12">
      ...
      {showTopBtn && (
        <button 
          onClick={scrollToTop} 
          className="fixed bottom-8 right-8 p-4 bg-emerald-600 text-white rounded-full shadow-2xl hover:bg-emerald-500 hover:-translate-y-1 transition-all z-50 group border-2 border-emerald-400"
          title="Jump to Top"
        >
      ...
      <div className="bg-white rounded-xl shadow-md border-2 border-green-100 overflow-hidden">
        <button 
          onClick={toggleMain}
          className="w-full bg-gradient-to-r from-green-800 to-emerald-800 text-white p-5 font-black flex items-center justify-between uppercase tracking-widest hover:from-green-700 hover:to-emerald-700 transition-colors"
        >
          <span className="flex items-center gap-3">
            <Trophy className="w-6 h-6 text-yellow-400" /> Knockout Stage
          </span>
                  ...
                  return (
                    <div key={stageInfo.id} className="border-2 border-slate-200 rounded-xl overflow-hidden transition-all shadow-sm">
                      <button
                        ...
                        className="w-full bg-white p-4 font-black text-slate-800 flex justify-between items-center hover:bg-green-50 hover:text-green-800 transition-colors uppercase tracking-wider"
                      >
                        <span className="flex items-center gap-3">
                          {stageInfo.name} 
      ...
      <div className="bg-white rounded-xl shadow-md border-2 border-green-100 overflow-hidden">
        <button 
          onClick={toggleGroup}
          className="w-full bg-gradient-to-r from-green-50 to-emerald-50 text-green-800 p-5 font-black flex items-center justify-between uppercase tracking-widest hover:from-green-100 hover:to-emerald-100 transition-colors border-b-2 border-green-100"
        >
          <span className="flex items-center gap-3">
            <Calendar className="w-6 h-6 text-emerald-500" /> Group Stage Matches
          </span>
                ...
                <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-lg px-2 py-1.5 w-full sm:w-auto">
                  ...
                  <select 
                    ...
                    onChange={e => {
                      ...
                      localStorage.setItem('worldCupGroupFilter', e.target.value);
                    }}
